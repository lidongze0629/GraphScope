...
def op_pre_process(op, op_result_pool, key_to_op):
    ...
    if op.op == types_pb2.FETCH_GREMLIN_RESULT:
        _pre_process_for_fetch_gremlin_result(op, op_result_pool, key_to_op)
    if op.op == types_pb2.CLOSE_INTERACTIVE_QUERY:
        _pre_process_for_close_interactive_query_op(op, op_result_pool, key_to_op)
    if op.op == types_pb2.SUBGRAPH:
        _pre_process_for_gremlin_to_subgraph_op(op, op_result_pool, key_to_op)
...
def _pre_process_for_close_interactive_query_op(op, op_result_pool, key_to_op):
    assert len(op.parents) == 1
    assert op.parents[0] in op_result_pool


def _pre_process_for_gremlin_to_subgraph_op(op, op_result_pool, key_to_op):
    assert len(op.parents) == 1
    assert op.parents[0] in op_result_pool
...
def _pre_process_for_fetch_gremlin_result(op, op_result_pool, key_to_op):
    assert len(op.parents) == 1
    assert op.parents[0] in op_result_pool
